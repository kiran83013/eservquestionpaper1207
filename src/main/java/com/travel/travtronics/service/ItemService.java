...
//		}
		...
		Pageable pageable = PageRequest.of(pageNo, pageSize);
		Page<Item> serviceMenuTypeData = esCustomRepository.fetchItemPagination(name,organizationId, pageable, sortBy, sortType);
		return new APIResponsePaging(HttpStatus.OK.value(), HttpStatus.OK.name(), serviceMenuTypeData.getContent(), new ArrayList<>(),
				serviceMenuTypeData.getNumber(), serviceMenuTypeData.getTotalElements(), serviceMenuTypeData.getTotalPages());
	}
}
